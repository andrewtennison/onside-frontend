...
BB.appRoutes = Backbone.Router.extend({
	...
	home: function(){
		...
		var status = parseFloat(on.m.app.get('user').status);
		(status === 1 || status === 0)
	...
	staticPage: function( name ){
		console.log('// Routes = "/"  (static):' + name);
		on.m.app.set({ selectedItemUID : 'static|'+name, selectedArticle: false });
	},
