...
exports.all = function(conf){
  ...
  everyauth.google.handleAuthCallbackError(handleOauthError);
	...
	.respondToLoginSucceed(function(res, user, data) {
		...
		if(user) {
	    	...
	    	console.log('respondToLoginSucceed: fail')
		}
	})
	.respondToRegistrationSucceed(function (res, user, data) { 
		console.log('respondToRegistrationSucceed')
		handleSuccessRedirect.call(this, res, data);
	});
  ...
  function fudgeFindUserByIdFunctionMiddleware(req, res, next) {
  	console.log('fudgeFindUserByIdFunctionMiddleware')
    everyauth.everymodule.findUserById(function(userId,callback){
      if (userId == req.session.user.id) {
        callback(null, req.session.user);
      }
      else {
        console.log('Need to get user with id: ' + userId + ' from API; existing session user: %j', req.session.user);
        callback(null, null);
      }
    });
    next();
  ...
  function authenticateWithPassword(email, password, req, res) {
  	console.log('authenticateWithPassword')
  ...
  function getTimeback(promise) {
    return function() { 
    	//promise.fail(['API Timeout'])
    	promise.fulfill(['API Timeout, this could be due to a problem with your internet connection'])
    };
  }
  ...
  function getCompletionCallback(promise, displayableErrorHandler) {
   	console.log('getCompletionCallback')

    return function(data, response) {
      if (response.statusCode == 200 || data.errorset ) {
        console.log('2. RESPONSE /////////////////////////////////////')
        if(data.errorset !== undefined) {
          console.log('3.ERROR /////////////////////////////////////');
          if( (data.errorset[0].error.message).indexOf('Duplicate') !== -1 ){
          	data.errorset[0].error.message = 'Email already registered. We are still in closed Beta, we will be sending out more invites soon. Thanks for your patience'
          }
